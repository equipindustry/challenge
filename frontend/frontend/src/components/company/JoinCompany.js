...
export default class LeaveCompany extends Component {
  ...
  handleSubmit(event) {
    ...
    const options = {
        url: 'http://localhost:3000/companies/join',
        method: 'POST',
        headers: {
  ...
  render() {
    return (
        ...
        <div>
        <Button className="btn btn-success" onClick={this.toggle}>Entrar</Button>
        <Modal show={this.state.modal}>
        <form onSubmit={this.handleSubmit}>
          <ModalHeader>
            <h3>¿Desea Ingresar a esta compañia?</h3>
          </ModalHeader>
          <ModalBody className="text-center">
          <div className="row">
                <h1 className="text-center">{this.state.companyname}</h1>
            </div>
          </ModalBody>
          <ModalFooter>
